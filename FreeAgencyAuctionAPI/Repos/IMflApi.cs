...
namespace FreeAgencyAuctionAPI.Repos
{
    ...
    public interface IMflApi
    {
        ...
        [Get("{year}/export?TYPE=league&L={leagueId}&APIKEY={apiKey}&JSON=1")]
        Task<LeagueRoot> GetBigLeagueObject([Path] int leagueId, [Path] int year, [Path] string apiKey);

        [Get("{year}/export?TYPE=rosters&L={leagueId}&APIKEY={apiKey}&FRANCHISE=&W=&JSON=1")]
        Task<RostersRoot> GetMflRostersForPlayerSalaries([Path] int leagueId, [Path] int year, [Path] string apiKey);

        [Get("{year}/export?TYPE=salaryAdjustments&L={leagueId}&APIKEY={apiKey}&JSON=1")]
        Task<SalaryAdjustmentsRoot> GetMflSalaryAdjustments([Path] int leagueId, [Path] int year, [Path] string apiKey);

        [Get("{year}/export?TYPE=freeAgents&L={leagueId}&APIKEY={apiKey}&POSITION=&JSON=1")]
        Task<FreeAgentsRoot> GetMflFreeAgents([Path] int leagueId, [Path] int year, [Path] string apiKey);

        [Get("{year}/export?TYPE=players&L={leagueId}&APIKEY={apiKey}&DETAILS=1&SINCE=&PLAYERS={ids}&JSON=1")]
        Task<MflPlayerDetailsRoot> GetMflPlayerDetails([Path] int leagueId, [Path] string ids, [Path] int year, [Path] string apiKey);

        [Get("{year}/import?TYPE=fcfsWaiver&L={leagueId}&DROP={playerId}&FRANCHISE_ID={franchiseId}")]
        Task<HttpResponseMessage> DropPlayer([Path] int leagueId, [Path] int playerId, [Path] string franchiseId, [Path] int year);
        ...
        Task<TransactionsRoot> GetLastYearWaiverTransactions([Path] int leagueId, [Path] string apiKey, [Path] int year);
        ...
        Task<MflAssetsRoot> GetFranchiseAssets([Path] int leagueId, [Path] int year, [Path] string apiKey);

        [Get("{year}/export?TYPE=salaries&L={leagueId}&APIKEY={apiKey}&JSON=1")]
        Task<MflSalariesParent> GetSalaries([Path] int leagueId, [Path] int year, [Path] string apiKey);
        ...
        Task<MflDraftResultsRoot> GetDraftResults([Path] int leagueId, [Path] int year, [Path] string apiKey);

        [Get("{year}/export?TYPE=futureDraftPicks&L={leagueId}&APIKEY={apiKey}&JSON=1")]
        Task<FutureDraftPicksRoot> GetFutureDraftPicks([Path] int leagueId, [Path] int year, [Path] string apiKey);

        [Get("{year}/export?TYPE=tradeBait&L={leagueId}&APIKEY={apiKey}&INCLUDE_DRAFT_PICKS=true&JSON=1")]
        Task<TradeBaitsParent> GetTradeBait([Path] int leagueId, [Path] string apiKey, [Path] int year);
    }
}
