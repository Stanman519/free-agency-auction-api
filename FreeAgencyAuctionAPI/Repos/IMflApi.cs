...
namespace FreeAgencyAuctionAPI.Repos
{
    ...
    public interface IMflApi
    {
        ...
        Task<LeagueRoot> GetBigLeagueObject([Path] int leagueId, [Path] int year);

        [Get("{year}/export?TYPE=rosters&L={leagueId}&APIKEY=&FRANCHISE=&W=&JSON=1")]
        Task<RostersRoot> GetMflRostersForPlayerSalaries([Path] int leagueId, [Path] int year);
        
        [Get("{year}/export?TYPE=salaryAdjustments&L={leagueId}&APIKEY=&JSON=1")]
        Task<SalaryAdjustmentsRoot> GetMflSalaryAdjustments([Path] int leagueId, [Path] int year);

        [Get("{year}/export?TYPE=freeAgents&L={leagueId}&APIKEY=&POSITION=&JSON=1")]
        Task<FreeAgentsRoot> GetMflFreeAgents([Path] int leagueId, [Path] int year);

        [Get("{year}/export?TYPE=players&L={leagueId}&APIKEY=&DETAILS=1&SINCE=&PLAYERS={ids}&JSON=1")]
        Task<MflPlayerDetailsRoot> GetMflPlayerDetails([Path] int leagueId, [Path] string ids, [Path] int year);
        ...
        Task<FutureDraftPicksRoot> GetFutureDraftPicks([Path] int leagueId, [Path] int year);
